...
export const MediumContent = ({ item, children}) => {
 ...
 {/*       <Attribute label="Event">
            <Link item={item}>
            {item?.event?.name || "Chyba dat!!"}
            </Link>
        </Attribute> */}
       {/*  <Attribute label="Název hodiny?">
            <Link item={item}>
            {item?.lessons?.name || "Chyba dat!!"}
            </Link>
        </Attribute> */}

{/*          <Attribute label={"Hodiny v plánu?"}>
                {item?.lessons?.name?.length > 0 ? item.lessons.name.map(les => les.lessons?.name).join(", ") : "Žádné hodiny!"}
        </Attribute> */}